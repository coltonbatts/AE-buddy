...
function emptyContext(): AEContext {
  return {
...
function normalizeContext(raw: unknown): AEContext {
  if (!raw || typeof raw !== "object") {
    return emptyContext();
  }
...
export async function loadAeContext(contextPath: string): Promise<AEContext> {
  try {
